...
def fill_tree(tree: ttk.Treeview, rows, columns, tags_func=None):
    ...
    for r in rows:
        vals = [str(r.get(col, "")).strip() if isinstance(r, dict) else str(r[i]).strip() for i, col in enumerate(columns)]
        tag = ()
        if tags_func:
            try:
                tag = (tags_func(r),)
...
if __name__ == "__main__":
    ...
    if not bsod_core.is_admin():
        warn = ttk.Label(root, text="⚠ Not running as Administrator — some logs/dumps and driver installs may be unavailable.",
                         style="Warn.TLabel")
        warn.pack(side="top", fill="x", padx=12, pady=(4, 2))
        ttk.Button(root, text="Run Elevated", command=run_elevated).pack(side="top", anchor="w", padx=12, pady=(0, 4))
    ...
    settings_frame.pack(side="top", fill="x", padx=12, pady=8)
    ttk.Label(settings_frame, text="Lookback (days):").pack(side="left", padx=(0, 4))
    ...
    def on_lookback_focus_out(event):
        try: bsod_core.set_lookback_days(int(lookback.get()))
        except Exception: pass
    lookback.bind("<Return>", on_lookback_focus_out)
    lookback.bind("<FocusOut>", on_lookback_focus_out)
    lookback.pack(side="left", padx=(0, 16))
    ttk.Label(settings_frame, text="BSOD window (±min):").pack(side="left", padx=(0, 4))
    ...
    def on_window_focus_out(event):
        try: bsod_core.set_window_min(int(winspin.get()))
        except Exception: pass
    winspin.bind("<Return>", on_window_focus_out)
    winspin.bind("<FocusOut>", on_window_focus_out)
    winspin.pack(side="left", padx=(0, 16))
    ...
    ttk.Label(errors_tab, text="Recent Errors & Warnings (System + Application)", style="Title.TLabel").pack(anchor="w", padx=10, pady=(8, 4))
    errors_recent_tree = ttk.Treeview(errors_tab, columns=("Time","Log","Provider","Id","LevelName","Message"), show="headings", height=8)
    ...
    errors_recent_tree.pack(fill="both", expand=True, padx=10, pady=(0, 6))
    ttk.Label(errors_tab, text="Top recurrent issues", style="Title.TLabel").pack(anchor="w", padx=10, pady=(8, 4))
    errors_top_tree = ttk.Treeview(errors_tab, columns=("Provider","EventId","Count","Level"), show="headings", height=8)
    ...
    errors_top_tree.pack(fill="both", expand=True, padx=10, pady=(0, 10))
    ...
    bugs_tree.pack(fill="both", expand=True, padx=10, pady=10)
    ...
    drivers_tree.pack(fill="both", expand=True, padx=10, pady=(10, 6))
    drv_btns = ttk.Frame(drivers_tab); drv_btns.pack(fill="x", padx=10, pady=(0, 10))
    btn_drivers_refresh = ttk.Button(drv_btns, text="Refresh", command=drivers_refresh); btn_drivers_refresh.pack(side="left", padx=4, pady=4)
    ttk.Button(drv_btns, text="Open Info Link", command=drivers_open_link).pack(side="left", padx=4, pady=4)
    ttk.Button(drv_btns, text="Install Selected (Admin)", command=drivers_install_selected).pack(side="left", padx=12, pady=4)

    # System Info tab
    sys_tab = ttk.Frame(nb); nb.add(sys_tab, text="System Info")
    tools = ttk.Frame(sys_tab); tools.pack(fill="x", padx=10, pady=(8, 4))
    ttk.Button(tools, text="Run SFC", command=run_sfc).pack(side="left", padx=4, pady=2)
    ttk.Button(tools, text="Run DISM", command=run_dism).pack(side="left", padx=4, pady=2)
    ttk.Button(tools, text="Run CHKDSK", command=run_chkdsk).pack(side="left", padx=4, pady=2)
    ttk.Button(tools, text="Memory Diagnostic", command=run_mdsched).pack(side="left", padx=12, pady=2)
    ...
    live_btn = ttk.Button(live_tab, text="Start Monitor", command=monitor_toggle); live_btn.pack(pady=8)
    live_box = scrolledtext.ScrolledText(live_tab, wrap="word", font=("Consolas", 10), padx=10, pady=10)
    live_box.pack(fill="both", expand=True, padx=10, pady=(0, 10)); live_box.config(state="disabled")

    # footer / signature
    footer = ttk.Label(root, text=f"{app_title} v{app_ver} — © 2026 {app_by}", anchor="w", relief="sunken", font=("Segoe UI", 8))
